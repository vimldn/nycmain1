...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      fetchData(DATASETS.dobViolations, `$where=boro='${borough}' AND block='${bbl.slice(1,6)}' AND lot='${bbl.slice(6)}'&$limit=800&$order=issue_date DESC`),
      fetchData(DATASETS.dobComplaints, `$where=boro='${borough}' AND block='${bbl.slice(1,6)}' AND lot='${bbl.slice(6)}'&$limit=400&$order=date_entered DESC`).catch(()=>[]),
      // DOB Job Filings - use boro not borough, values are strings
      fetchData(DATASETS.dobJobFilings, `$where=boro='${borough}' AND block='${bbl.slice(1,6)}' AND lot='${bbl.slice(6)}'&$limit=300&$order=filing_date DESC`).catch(()=>[]),
      fetchData(DATASETS.dobPermitsIssued, `$where=boro='${borough}' AND block='${bbl.slice(1,6)}' AND lot='${bbl.slice(6)}'&$limit=200`).catch(()=>[]),
      fetchData(DATASETS.dobSafety, `$where=boro='${borough}' AND block='${bbl.slice(1,6)}' AND lot='${bbl.slice(6)}'&$limit=150`),
      fetchData(DATASETS.dobEcb, `$where=boro='${borough}' AND block='${bbl.slice(1,6)}' AND lot='${bbl.slice(6)}'&$limit=300`),
      fetchData(DATASETS.dobVacates, `$where=boro='${borough}' AND block='${bbl.slice(1,6)}' AND lot='${bbl.slice(6)}'&$limit=30`).catch(()=>[]),
