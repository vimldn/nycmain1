...
function renderSection(
  ...
  switch (section.type) {
    ...
    case 'h2':
      return (
        <h2 key={index} style={{fontFamily:'"Bebas Neue",sans-serif',fontSize:'26px',letterSpacing:'.04em',color:'#0a0a0a',marginTop:'40px',marginBottom:'16px'}}>
          {section.heading}
        </h2>
      )
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#f5f5f5]">
                {section.rows[0].map((cell, ci) => (
                  <th key={ci} className="px-4 py-3 text-left text-xs font-semibold text-[#555] uppercase border-b-2 border-[#0a0a0a]" style={{fontFamily:'"Space Mono",monospace',letterSpacing:'.08em'}}>
                    {cell}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {section.rows.slice(1).map((row, ri) => (
                <tr key={ri} className={ri % 2 === 0 ? 'bg-white' : 'bg-[#f5f5f5]/50'}>
                  {row.map((cell, ci) => (
                    <td key={ci} className="px-4 py-3 text-base text-[var(--text-secondary)] border-b border-[#1e293b]/50 last:border-b-0">
                      {ci === 0 ? <span className="font-medium text-[#0a0a0a]">{cell}</span> : cell}
                    </td>
                  ))}
                </tr>
              ))}
      ...
      return (
        <div key={index} className="mb-8">
          <div className="flex items-baseline gap-3 mb-3">
            <span className="flex-shrink-0 w-7 h-7 bg-[#e0f5f2] border border-[#0b8a7a] flex items-center justify-center text-xs font-black text-[#0b8a7a]">
              {section.stepNumber}
            </span>
    ...
    case 'list':
      return (
        <ul key={index} className="my-6 space-y-3">
          {section.items?.map((item, i) => (
            <li key={i} className="flex items-start gap-3 text-base text-[var(--text-secondary)]">
              <span className="mt-2 w-1.5 h-1.5 bg-[#0b8a7a] flex-shrink-0" />
              <span className="leading-relaxed">{item}</span>
            </li>
          ))}
        </ul>
    ...
    case 'faq':
      return (
        <div key={index} className="my-10">
          <h2 style={{fontFamily:'"Bebas Neue",sans-serif',fontSize:'24px',letterSpacing:'.04em',color:'#0a0a0a',marginBottom:'20px'}}>
          ...
          <div className="space-y-2">
            {section.items?.map((item, i) => (
              ...
              >
                <summary className="flex items-center justify-between gap-4 px-5 py-4 cursor-pointer list-none select-none hover:bg-[#f5f5f5] transition-colors">
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-12">
          ...
          <article className="min-w-0">
            ...
            <nav className="flex items-center gap-2 text-xs text-[#475569] mb-8">
              <Link href="/" className="hover:text-[#0b8a7a] transition-colors">Home</Link>
              <span>/</span>
              <Link href="/guides" className="hover:text-[#0b8a7a] transition-colors">Guides</Link>
              <span>/</span>
              {category && (
                <>
                  <span className="text-[#0b8a7a]">{category.title}</span>
                  <span>/</span>
                </>
              )}
            ...
            {guide.relatedServicePages.length > 0 && (
              <div className="mt-8">
                <h3 className="text-sm font-semibold text-[#0a0a0a] mb-3" style={{fontFamily:'"Bebas Neue",sans-serif',fontSize:'16px',letterSpacing:'.06em'}}>Find a {guide.serviceName.toLowerCase()} near you</h3>
                <div className="flex flex-wrap gap-2">
                  {guide.relatedServicePages.map(sp => (
                    <Link
                      key={sp.href}
                      href={sp.href}
                      className="px-3 py-1.5 border border-[#e0e0e0] text-xs text-[#555] hover:text-[#0b8a7a] hover:border-[#0b8a7a] transition-colors" style={{fontFamily:'"Space Mono",monospace',letterSpacing:'.06em'}}
                    >
                      {sp.label}
                    </Link>
                  ))}
              ...
              <div className="mt-12 pt-8 border-t border-[var(--border-primary)]">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {guide.furtherReading.map(post => (
                    ...
                    >
                      <div className="flex-shrink-0 w-6 h-6 bg-[#e0f5f2] flex items-center justify-center mt-0.5">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#0b8a7a]">
              ...
              <div className="mt-10">
                ...
                <div className="space-y-3">
                  {guide.govLinks.map(link => (
                    ...
                    >
                      <div className="flex-shrink-0 w-7 h-7 bg-[#e0f5f2] flex items-center justify-center mt-0.5">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#0b8a7a]">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="text-sm font-semibold text-[#0a0a0a] group-hover:text-[#0b8a7a] transition-colors">{link.label}</span>
                        ...
                        <p className="text-xs text-[#555] leading-relaxed">{link.description}</p>
                      </div>
                    </a>
                  ))}
                </div>
