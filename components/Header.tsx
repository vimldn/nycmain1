...
export default function Header({ showSearch = false, searchPlaceholder = 'Search any NYC address...' }: HeaderProps) {
  ...
  useEffect(() => { setMobileOpen(false); setServicesOpen(false); setLocationsOpen(false) }, [pathname])
    ...
    <>
      <header style={{ position:'fixed', top:0, left:0, right:0, zIndex:50, background:'#fff', borderBottom:'3px solid #0a0a0a', height:'60px', display:'flex', alignItems:'center' }}>
        <div style={{ width:'100%', maxWidth:'1400px', margin:'0 auto', padding:'0 24px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'24px' }}>
          ...
          {showSearch && (
            <form onSubmit={handleSearch} style={{ flex:1, maxWidth:'480px' }} className="hidden sm:block">
          ...
          <button onClick={()=>setMobileOpen(!mobileOpen)} aria-label="Toggle menu"
            style={{ background:'none', border:'2px solid #0a0a0a', padding:'6px 10px', cursor:'pointer', display:'flex', alignItems:'center' }}
            className="md:hidden">
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div style={{ position:'fixed', top:'60px', left:0, right:0, bottom:0, background:'#fff', zIndex:49, overflowY:'auto', borderTop:'2px solid #0a0a0a' }} className="md:hidden">
          <form onSubmit={handleSearch} style={{ padding:'16px', borderBottom:'2px solid #0a0a0a' }}>
            <div style={{ display:'flex', border:'2px solid #0a0a0a' }}>
          ...
          <Link href="/" onClick={()=>setMobileOpen(false)}
            style={{ display:'block', fontFamily:'"Space Mono", monospace', fontSize:'11px', letterSpacing:'0.1em', textTransform:'uppercase', padding:'16px 20px', borderBottom:'1px solid #e0e0e0', textDecoration:'none', color:'#0a0a0a' }}>Home</Link>
